...
public class NPCDeserializer extends StdDeserializer<NPC.Global> {
    ...
    @Override
    public NPC.Global deserialize(JsonParser p, DeserializationContext context) throws IOException {
        ...
        Optional<NPC.Global> optionalNPC = NPCLib.getInstance().grabGlobalNPC(Main.getInstance(), npcSimpleCode);
        NPC.Global npc;
        if (optionalNPC.isEmpty()) {
            npc = NPCLib.getInstance().generateGlobalNPC(Main.getInstance(), npcSimpleCode, location);
        } else {
            npc = optionalNPC.get();
            Main.bukkitSync(() -> npc.teleport(location));
        }
        ...
        if (node.has("equip")) {
            ...
            if (equip.has("helmet")) {
                npc.setItem(NPC.Slot.HEAD, mapper.treeToValue(equip.get("helmet"), ItemStack.class));
            }
            if (equip.has("chest")) {
                npc.setItem(NPC.Slot.CHEST, mapper.treeToValue(equip.get("chest"), ItemStack.class));
            }
            if (equip.has("leggings")) {
                npc.setItem(NPC.Slot.LEGS, mapper.treeToValue(equip.get("leggings"), ItemStack.class));
            }
            if (equip.has("boots")) {
                npc.setItem(NPC.Slot.FEET, mapper.treeToValue(equip.get("boots"), ItemStack.class));
            }
        }
        if (node.has("skin")) {
