...
public class InventoryLarge extends FastInv {
    ...
    public InventoryLarge(Player player, @NotNull Shop shop, @NotNull List<Trade> trades, boolean fill) {
        ...
        if (fill) {
            ...
            //  TODO: Something better xD
            this.tradeMode = TradeMode.SHULKER;
        }
        ...
        for (Trade trade : trades) {
            ...
            if (pageTradeCount > ShopsManager.INVENTORY_LARGE_TRADES_PER_PAGE) {
    ...
    private void pageButtons() {
        if (this.currentPage > 1 ) {
            setItem(getInventory().getSize()-9, Buttons.PREVIOUS.get(), event -> {
                if (this.currentPage > 1) {
                    this.currentPage--;
                }
                updatePageContent();
            ...
            setItem(getInventory().getSize()-9, Buttons.PANE_BLACK.get());
        }
        if (pagesTrades.containsKey(currentPage + 1)) {
            setItem(getInventory().getSize(), Buttons.NEXT.get(), event -> {
                if (this.currentPage >= 64) return;
                this.currentPage++;
                updatePageContent();
            });
        } else {
            setItem(getInventory().getSize(), Buttons.PANE_BLACK.get());
        }
    }
    ...
    private void displayTrade(int pagePosition, @NotNull Trade trade) {
        int slotOffset = (pagePosition + 1) * 9;
        if (slotOffset >= 9) {
            slotOffset -= 9;
        }
        if (pagePosition >= 6) {
            slotOffset = ((pagePosition - 6) * 9) + 5;
        }
        setItem(slotOffset, trade.getFirstItem().clone());
        if (fill) setItem(1 + slotOffset, Buttons.HEAD_LEFT.get());
        if (fill) setItem(2 + slotOffset, Buttons.HEAD_LEFT.get());
        setItem(3 + slotOffset, trade.getResultItem().clone(), event -> {
